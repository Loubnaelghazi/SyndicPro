...
import { HiBell, HiExclamationTriangle, HiUserCircle, HiBars3 } from "react-icons/hi2";
...
export default function Header() {
    ...
    function hideElement ()  {
        if(display=="none"){setDesplay("block")}
        else{setDesplay("none")}
      };
    

    
    return (
    <div className="bg-white text-primary-color rounded-20 h-min mr-6 shadow-csh">
        <div className="  flex flex-row mt-4 p-4 justify-between ">
            <div className="flex flex-row justify-between">
                <a className="lg:hidden duration-500 text-2xl ml-2 mr-5 hover:text-third-color hover:bg-primary-color rounded-md">
                    <button onClick={hideElement}><HiBars3/></button>
                </a>
        ...
        <div style={{display: `${display}`}} className=" transition-all lg:hidden duration-400">
